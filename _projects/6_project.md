...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/1.png" title="图 1" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/2.png" title="图 2" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/3.png" title="图 3" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/4.png" title="图 4" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
#### 理论建模
假设电场使液晶分子轴方向偏转一个角度 $$ \theta $$，$$ \theta $$ 随电压 $$ V $$ 变化的函数为[[16]]:
$$ \theta = \begin{cases} 0 & \text{when } V \leq V_{th} \\ \frac{\pi}{2} - 2 \tanh\left[\exp\left(\frac{V_{th}-V}{V_{0}}\right)\right] & \text{when } V > V_{th} \end{cases} $$

其中 $$ V_{th} $$ 是间值电压，$$ V_{0} $$ 是饱和电压。平行于分子长轴的折射率为 $$ n_{\|} $$，垂直方向的折射率为 $$ n_{\perp} $$。随着分子轴方向的旋转，根据折射率得出入射光偏振方向上的折射率为:
$$ n(e) = n(\theta) = \frac{n_{\|} n_{\perp}}{\sqrt{n_{\perp}^{2} \cos^{2} \theta + n_{\|} \sin^{2} \theta}} $$

把液晶盒看作一个相位延迟片，可以得到液晶盒（平行面排列）的琼斯矩阵[[15]]:
$$ J = \begin{bmatrix} 1 & 0 \\ 0 & e^{-i \delta} \end{bmatrix} $$

其中，$$ \delta = \frac{2 \pi \Delta n}{\lambda} $$，$$ \Delta n = n(e) - n(o) $$。$$ \Delta n $$ 是双折射率，它与温度的关系为[[17]]:
$$ \Delta n(T) = \Delta n_{0}\left(1-\frac{T}{T_{c}}\right)^{\beta} $$

$$ \Delta n_{0} $$ 是液晶在绝对零度下的双折射率，$$ T_{c} $$ 是液晶的临界温度，$$ \beta $$ 是液晶的物理特性，可以通过实验测得。

定义 $$ I_{\text{in}} $$ 是入射的偏振光的光强，经过电机旋转的偏振片后有:
$$ I_{\text{in}} = I_{0} \cos^{2} \omega \Delta t $$

设置计算机计算的反馈公式为:
$$ V_{n+1} = C_{1} I_{\text{out}} + C_{2} V_{n} $$

设起偏器、验偏器的透光轴与X轴的夹角分别为 $$ \alpha $$ 和 $$ \beta $$，液晶的相位延迟为 $$ \delta $$，从验偏器射出的偏振光的琼斯矩阵为:
$$ E_{\text{out}} = \begin{bmatrix} E_{\text{outx}} \\ E_{\text{outyy}} \end{bmatrix} = J_{A} J_{C} E_{i} = \frac{E_{\text{in}}}{2} \begin{bmatrix} \cos \beta \cos \alpha + \sin \beta \sin \alpha e^{-i \delta} \\ -\sin \beta \cos \alpha + 2 \sin \alpha \cos \beta e^{-i \delta} \end{bmatrix} $$

其中 $$ E_{i} = \begin{bmatrix} E_{\text{in}} \cos \alpha \\ E_{\text{in}} \sin \alpha \end{bmatrix} $$ 是光线经过起偏器的琼斯矩阵。

$$ J_{C} = \begin{bmatrix} 1 & 0 \\ 0 & e^{-i \delta} \end{bmatrix} $$ 是液晶的琼斯矩阵。

$$ J_{A} = \begin{bmatrix} \cos \beta & \sin \beta \\ -\sin \beta & \sin \beta \end{bmatrix} $$ 是验偏器的琼斯矩阵。

所以，由验偏器射出的光强为:
$$ I_{\text{out}} = \left|E_{\text{out}}\right|^{2} = I_{\text{in}} \left|\begin{array}{c} \cos \beta \cos \alpha + \sin \beta \sin \alpha e^{-i \delta} \\ -\sin \beta \cos \alpha + 2 \sin \alpha \cos \beta e^{-i \delta} \end{array}\right|^{2} $$

其中 $$ \omega $$ 是电机控制的偏振片旋转的角速度，$$ \Delta t $$ 是开始测量之后的时间间隔。最后形成了一个完整的混池体系:
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/5.png" title="图 5" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/6.png" title="图 6" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/7.png" title="图 7" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/8.png" title="图 8" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/9.png" title="图 9" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/10.png" title="图 10" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/11.png" title="图 11" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/13.png" title="图 13" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/15.png" title="图 15" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/chaos/16.png" title="图 16" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
[1] F. T. Arecchi, E. Courtens, R. Gilmore, and H. Thomas. Atomic coherent states in quantum
optics. Phys. Rev. A, 6(6):2211–2237, 1972.
...
[4] H. Gibbs, S. L. McCal1, and T. N. C. Venkatesan. Differential gain and bistability using a
sodium-filled fabry-perot interferometer. Phys. Rev. Lett., 36:1135, 1976.
...
[6] A. L. Lewis, M. T. Turvey, and R. E. Shaw. Dynamics and chaos in an acousto-optic system.
J. Opt. Soc. Am. B, 8(12):2396–2406, 1991.

[7] E. N. Lorenz. Deterministic non-periodic flow. J. Atmos. Sci., 20(3):130–141, 1963.

[8] M. C. Soriano, J. García-Ojalvo, C. R. Mirasso, and I. Fischer. Chaos control and synchronization in an optoelectronic oscillator with optical feedback. Phys. Rev. E.

[9] J. P. Toomey, K. A. Shore, B. D. O. Anderson, and K. D. Choquette. Secure communication
using chaotic semiconductor lasers. IEEE J. Quantum Electron., 37(3):343–351, 2001.

[10] 何军, 张然, and 鲁兴海. 快速高双折射率向列相液晶的分子设计与模拟计算. 液晶与显示, 24.

[11] 叶文江, 张志东, and 邢红玉. 平行排列液晶盒的挠曲电效应及一级 fr(e)edericksz 转变. 液晶与
显示, 20(3):229–234, 2005.
...
[16] 王思齐, 孟颖, and 高文莉. 不同取向的向列型液晶盒的光电响应曲线. 物理实验, 32(9):12–17,
2012.
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0 mx-auto" style="max-width: 600px;">
        {% include figure.html path="assets/img/chaos/17.jpg" title="图 17" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
